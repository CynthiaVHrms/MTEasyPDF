        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="mt-1 text-sm leading-6 text-emerald-800">
              El documento Word se descargó correctamente y todas las
              imágenes solicitadas pudieron relacionarse con sus
              carpetas.
            </p>
          </div>
        </div>
        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="mt-1 text-sm leading-6 text-amber-900">
              El documento Word se descargó correctamente. Algunas
              evidencias no pudieron encontrarse o insertarse; revisa
              el detalle antes de entregar el documento.
            </p>
          </div>
        </div>
      </div>
        ...
        {diagnostics.imagenes_no_encontradas.length > 0 && (
          <details className="group mt-5 rounded-2xl border border-amber-200 bg-amber-50/60">
            ...
            <div className="max-h-80 space-y-3 overflow-y-auto border-t border-amber-200 p-4">
              {diagnostics.imagenes_no_encontradas.map(
                (item, index) => (
                  <article
          ...
          <details className="group mt-4 rounded-2xl border border-violet-200 bg-violet-50/60">
            <summary className="flex cursor-pointer list-none items-center justify-between gap-4 px-5 py-4 text-sm font-bold text-violet-950">
              <span>
                ...
                {
                  diagnostics.clasificaciones_no_encontradas
                    .length
                }
                )
              </span>
            ...
            <div className="max-h-72 space-y-2 overflow-y-auto border-t border-violet-200 p-4">
              {diagnostics.clasificaciones_no_encontradas.map(
                (item, index) => (
                  <div
        ...
        {diagnostics.imagenes_omitidas.length > 0 && (
          <details className="group mt-4 rounded-2xl border border-rose-200 bg-rose-50/60">
            ...
            <div className="max-h-72 space-y-2 overflow-y-auto border-t border-rose-200 p-4">
              {diagnostics.imagenes_omitidas.map(
                (item, index) => (
                  <div
...
export function ProtocolsSection() {
  ...
  const [evidenceZip, setEvidenceZip] =
    useState<File | null>(null);
  ...
  async function handleGenerateFolders() {
    ...
    if (!excelFile) {
      setError(
        "Selecciona la plantilla Excel para generar las carpetas.",
      );
    ...
    try {
      ...
      setMessage(
        "Estructura de carpetas generada correctamente.",
      );
    } catch (err) {
      setError(
  ...
  async function handleGenerateDocument() {
    ...
    if (!excelFile || !evidenceZip) {
      setError(
        "Selecciona la plantilla Excel y el ZIP de evidencias.",
      );
      ...
      while (!controller.signal.aborted) {
        ...
        const resultDiagnostics =
          status.diagnostics_ready
            ? await getC5DocumentDiagnostics(
                job.job_id,
                controller.signal,
              )
            : null;
        ...
        const hasObservations =
          ...
          (resultDiagnostics.resumen
            .imagenes_no_encontradas > 0 ||
            resultDiagnostics.resumen
              .imagenes_omitidas_por_dano > 0 ||
            resultDiagnostics.resumen
              .clasificaciones_no_encontradas > 0);
    ...
    <div className="space-y-6">
      <section className="rounded-4xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-500">
          Genera la estructura de carpetas desde una plantilla
          Excel y después crea el documento Word del protocolo
          utilizando el ZIP de evidencias.
        </p>
      </section>
      ...
      <section className="grid gap-6 xl:grid-cols-2">
        <article className="rounded-4xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="mb-6 flex items-start gap-4">
            ...
            <div>
              ...
              <p className="mt-1 text-sm leading-6 text-slate-500">
                Descarga la plantilla, llena la información
                requerida y súbela para generar las carpetas.
              </p>
            </div>
          </div>
          ...
          <div className="mb-5 rounded-2xl border border-blue-100 bg-blue-50 p-4">
            ...
            <p className="mt-1 text-xs leading-5 text-blue-700">
              Utiliza esta plantilla como base para capturar la
              información de sitios y clasificaciones.
            </p>
        ...
        <article className="rounded-4xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="mb-6 flex items-start gap-4">
            <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-slate-800 text-2xl text-white">
              📝
            </div>
            ...
            <div>
              ...
              <p className="mt-1 text-sm leading-6 text-slate-500">
                Sube el ZIP de evidencias y genera el documento
                final con la plantilla oficial configurada en el
                sistema.
              </p>
            </div>
          </div>

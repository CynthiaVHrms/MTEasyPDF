...
def dividir_en_bloques(
    ...
    """
    Divide una lista en bloques del tamaño indicado.

    En el protocolo C5 se utiliza para limitar cada página a un máximo
    de cuatro evidencias: dos columnas por fila y máximo dos filas por
    bloque de clasificación.
    """

    if tamano_bloque <= 0:
        raise ValueError("El tamaño del bloque debe ser mayor a cero.")

    return [
        elementos[position : position + tamano_bloque]
        for position in range(0, len(elementos), tamano_bloque)
    ]


def construir_pares_de_evidencias(
    evidencias: list[dict[str, object]],
) -> list[dict[str, object]]:
    """
    Convierte una lista de evidencias en pares para la tabla de dos columnas
    utilizada por la plantilla Word.
    """

    pares: list[dict[str, object]] = []

    for position in range(0, len(evidencias), 2):
        pares.append(
            {
                "izq": evidencias[position],
                "der": (
                    evidencias[position + 1]
                    if position + 1 < len(evidencias)
                    else None
                ),
            }
        )
...
def agregar_bloques_de_clasificacion_paginados(
    *,
    lista_clasificaciones: list[dict[str, object]],
    titulos_para_paginacion: list[str],
    clasificacion: object,
    evidencias: list[dict[str, object]],
    maximo_imagenes_por_pagina: int = 4,
) -> None:
    """
    Agrega una clasificación al contexto de docxtpl respetando la regla visual
    del documento: máximo cuatro evidencias por hoja.

    Si una clasificación contiene más de cuatro evidencias, se divide en varios
    bloques. Cada bloque repite el título de la clasificación y después, en una
    etapa posterior, se inserta un salto de página antes de cada bloque nuevo.
    """

    titulo = str(clasificacion)
    bloques = dividir_en_bloques(
        evidencias,
        maximo_imagenes_por_pagina,
    )
        ...
        lista_clasificaciones.append(
            {
                ...
                "pares": construir_pares_de_evidencias(bloque),
            }
        )
        titulos_para_paginacion.append(titulo)


def _activar_cant_split_en_fila(row) -> None:
    """
    Evita que Word divida una fila de la tabla entre dos páginas.

    Esto ayuda a que una evidencia completa —imagen y datos— permanezca junta.
    """
...
def aplicar_reglas_de_paginacion_word(
    docx_path: Path,
    titulos_clasificaciones: list[str],
) -> None:
    """
    Ajusta el documento generado para evitar títulos huérfanos y limitar
    visualmente cada página a bloques de máximo cuatro evidencias.

    Reglas aplicadas:
    - El primer bloque de evidencias permanece junto al encabezado del sitio.
    - Cada bloque posterior inicia en una página nueva.
    - Los títulos de clasificación se mantienen junto al contenido siguiente.
    - Las filas de evidencias no se dividen entre páginas.
    """
    ...
    titulos_normalizados = {
        ...
        for titulo in titulos_clasificaciones
        if normalizar_texto(titulo)
    }

    clasificaciones_encontradas = 0

    for paragraph in documento.paragraphs:
        texto_normalizado = normalizar_texto(paragraph.text)

        if texto_normalizado not in titulos_normalizados:
            continue

        paragraph.paragraph_format.keep_with_next = True
        paragraph.paragraph_format.keep_together = True

        if clasificaciones_encontradas > 0:
            paragraph.paragraph_format.page_break_before = True

        clasificaciones_encontradas += 1

    for table in documento.tables:
        for row in table.rows:
            _activar_cant_split_en_fila(row)
...
def generate_protocol_document(
    ...
    *,
    ...
    total_imagenes_esperadas = len(hoja2)
    ...
    def report_processing_progress(
        *,
        ...
        if total_imagenes_esperadas > 0:
            work_ratio = imagenes_procesadas / total_imagenes_esperadas
        else:
            work_ratio = current_site / max(1, total_sitios)
    ...
    for index, fila_sitio in hoja1.iterrows():
        ...
        titulos_clasificaciones_paginacion: list[str] = []
        ...
        for clasificacion in clasificaciones_unicas:
            ...
            agregar_bloques_de_clasificacion_paginados(
                lista_clasificaciones=lista_clasificaciones,
                titulos_para_paginacion=titulos_clasificaciones_paginacion,
                clasificacion=clasificacion,
                evidencias=lista_individual,
                maximo_imagenes_por_pagina=4,
            )
        ...
        aplicar_reglas_de_paginacion_word(
            docx_path=temporal_path,
            titulos_clasificaciones=titulos_clasificaciones_paginacion,
        )
    ...
    for document_index, archivo in enumerate(archivos_generados[1:], start=2):
        ...
        _emit_progress(
            ...
            message=f"Uniendo documento {document_index} de {total_archivos}.",
            current_site=total_sitios,
            total_sites=total_sitios,
            processed_images=imagenes_procesadas,
            total_images=total_imagenes_esperadas,
